        ...
        <div>
          {
            list.map((data, i) => {
              ...
              const dis = `${di.getFullYear()}. ${di.getMonth() + 1 >= 10 ? di.getMonth() + 1 : `0${di.getMonth() + 1}`}. ${di.getDate() >= 10 ? di.getDate() : `0${di.getDate()}`}`;
              const dou = new Date(data["dateDelivery"]);
              const dous = `${dou.getFullYear()}. ${dou.getMonth() + 1 >= 10 ? dou.getMonth() + 1 : `0${dou.getMonth() + 1}`}. ${dou.getDate() >= 10 ? dou.getDate() : `0${dou.getDate()}`}`;
              return (
                <Link href={`/uniform?id=${data.id}`} key={String(i)}>
                  <a className={styles["table-section-row"]}>
                    <div className={styles["table-shopped-section1"]}>{data.id}</div>
                    <div className={styles["table-shopped-section2"]}>
                      <div style={{ backgroundImage: `url(${data.images[0]})` }}/>
                    </div>
                    <div className={styles["table-shopped-section3"]}>{data["filter-school"]}</div>
                    <div className={styles["table-shopped-section4"]}>{data["filter-clothType"].join(" / ")}</div>
                    <div className={styles["table-shopped-section5"]}>{data["giverName"]}</div>
                    <div className={styles["table-shopped-section6"]}>{`${data["receiverName"]}`}</div>
